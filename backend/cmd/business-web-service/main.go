...
func main() {
	...
	// -----------------------------
	logger, err := mylogger.New("business-web-service", "debug") // could be cfg.App.LogLevel if you add one
	if err != nil {
	...
	fs := http.FileServer(http.Dir("/app/web/business"))
	http.Handle("/", fs)

	logger.Info().Msgf("Starting business-web-service service on port %s", cfg.Srv.BusinessWebServicePort)
	if err := http.ListenAndServe(":"+cfg.Srv.BusinessWebServicePort, nil); err != nil {
		logger.Fatal().Err(err).Msg("Failed to start business-web-service")
	}

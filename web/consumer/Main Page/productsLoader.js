...
document.addEventListener("DOMContentLoaded", () => {
  // Один раз подгружаем доп. товары с бэкенда
  loadProducts().then(() => {
    // после загрузки перезапускаем фронтовую фильтрацию, если она есть
    try {
      if (typeof applyAll === "function") {
        const qInput = document.getElementById("q");
        const currentQuery = qInput ? qInput.value : "";
        applyAll(currentQuery);
      }
    } catch (e) {
      console.warn("Не удалось обновить фильтры после загрузки продуктов", e);
    }
  });
  
  // никаких обработчиков searchForm / sortSelect здесь больше не вешаем
});
